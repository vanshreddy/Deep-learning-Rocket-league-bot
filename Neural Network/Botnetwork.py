...
class botNN:
    ...
    def buildModel(self):
        ...
        steerModel = layers.Dense(512, activation=lrelu)(steerModel)
        steerModel = layers.Dense(512, activation=lrelu)(steerModel)
        steerOutput = layers.Dense(2,activation='softmax', name='steerOutput')(steerModel)
        ...
        eulerModel = layers.Dense(512, activation=lrelu)(eulerModel)
        eulerOutput = layers.Dense(3,activation='softmax', name="eulerOutput")(eulerModel)

        jumpModel = layers.Dense(128, activation='relu')(archLayer)
        jumpModel = layers.Dense(256, activation='relu')(jumpModel)
        jumpOutput = layers.Dense(2, activation='sigmoid', name="jumpOutput")(jumpModel)
